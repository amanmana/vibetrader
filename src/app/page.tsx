...
export default function VibeTrader() {
    ...
    if (action === 'BUY') {
      if (trend === 'Bullish' && confidence === 'High') {
        return {
          ...
          badge: 'bg-emerald-500 text-slate-950 shadow-emerald-500/20',
  ...
  if (isCheckingAuth) {
    return (
      <main className="relative min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center font-sans">
        <div className="flex flex-col items-center gap-3">
          <span className="w-10 h-10 border-3 border-slate-800 border-t-blue-500 rounded-full animate-spin" />
          <p className="text-slate-500 text-sm font-mono">Loading security settings...</p>
        </div>
      </main>
    );
  }
  ...
  if (!isLoggedIn) {
    return (
      <main className="relative min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center font-sans px-6 overflow-hidden">
        ...
        <div className="relative w-full max-w-md bg-slate-900/40 border border-slate-800/80 p-8 rounded-3xl backdrop-blur-xl shadow-2xl space-y-8 animate-in fade-in zoom-in duration-500">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-slate-950/80 border border-slate-850 focus:border-blue-500/80 focus:ring-1 focus:ring-blue-500/30 p-4 rounded-xl outline-none text-slate-100 placeholder-slate-700 transition text-center font-mono tracking-widest text-base"
                autoFocus
              />
            </div>
  ...
  return (
    <main className="relative min-h-screen bg-[#0B1120] text-slate-100 overflow-x-hidden font-sans pb-20">
      ...
      <div className="relative max-w-5xl mx-auto px-6 pt-16">
        ...
        <div className="flex flex-col items-center text-center mb-12 relative">
          <button
            ...
            className="absolute right-0 top-0 bg-slate-900 hover:bg-slate-800 border border-slate-850 hover:border-slate-700 p-2.5 rounded-xl text-slate-400 hover:text-slate-200 transition cursor-pointer flex items-center gap-2 text-xs font-semibold"
            title="Log Out"
          >
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-slate-400 text-xs font-medium mb-4 backdrop-blur-md">
        ...
        {/* Input Card */}
        <div className="bg-slate-900/40 border border-slate-800/80 p-6 md:p-8 rounded-3xl backdrop-blur-xl shadow-2xl mb-8">
          <div className="max-w-xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input 
                  type="text"
                  value={ticker}
                  className="w-full bg-slate-950/80 border border-slate-800 focus:border-blue-500/80 focus:ring-1 focus:ring-blue-500/30 p-4 pl-12 rounded-2xl outline-none text-slate-100 placeholder-slate-600 transition font-mono uppercase tracking-wider text-base"
            ...
            <div className="flex flex-wrap items-center gap-2 mt-5">
              <span className="text-slate-600 text-xs font-medium mr-1">Latest Counter:</span>
              {latestCounters.map((popTicker) => (
                <button
                  ...
                  className="bg-slate-950/50 hover:bg-slate-800 border border-slate-850 hover:border-slate-700 px-3 py-1.5 rounded-lg text-xs font-mono text-slate-400 transition cursor-pointer"
                >
                  {popTicker}
                </button>
              ))}
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl backdrop-blur-xl shadow-2xl mb-8 overflow-hidden">
          {/* Tabs */}
          <div className="flex border-b border-slate-800/60 bg-slate-900/50">
            <button
              ...
              className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition border-b-2 ${
                activeTab === 'watchlist' ? 'border-amber-400 text-amber-400 bg-slate-800/30' : 'border-transparent text-slate-500 hover:bg-slate-800/50 hover:text-slate-300'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition border-b-2 ${
                activeTab === 'screener' ? 'border-blue-400 text-blue-400 bg-slate-800/30' : 'border-transparent text-slate-500 hover:bg-slate-800/50 hover:text-slate-300'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-4 text-sm font-bold flex items-center justify-center gap-2 transition border-b-2 ${
                activeTab === 'calculator' ? 'border-emerald-400 text-emerald-400 bg-slate-800/30' : 'border-transparent text-slate-500 hover:bg-slate-800/50 hover:text-slate-300'
              }`}
            >
                          ...
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="flex items-center gap-2">
                                <a href={`https://www.tradingview.com/chart/S83uhZmn/?symbol=${res.ticker}`} target="_blank" rel="noopener noreferrer" className="font-mono font-bold text-2xl text-slate-100 hover:text-blue-400 transition cursor-pointer">{res.ticker}</a>
                              ...
                              <span className="block text-sm text-slate-400 mt-1">{getCurrencySymbol(res.ticker)} {price ? price.toFixed(2) : '-'}</span>
                              {name && <span className="block text-xs text-slate-600 line-clamp-1 mt-1">{name}</span>}
                            </div>
                            <div className="flex items-center gap-2">
                              <button
                                onClick={(e) => {
                              ...
                              <span className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider ${isBuy ? 'bg-emerald-500 text-slate-950' : 'bg-amber-500 text-slate-950'}`}>
                                {action}
                              </span>
                            </div>
                          </div>
              ...
              <>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                  <div>
                    ...
                    <p className="text-slate-500 text-xs mt-1">Tracked tickers for daily scanning</p>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      disabled={isScanning}
                      className="bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      title="Scan All"
                    >
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {watchlist.map((sym) => (
                    <div key={sym} className="group relative flex items-center bg-slate-950/80 border border-slate-800 rounded-lg overflow-hidden">
                      <button
                        onClick={() => {
                          setTicker(sym);
                          analyzeTicker(sym);
                        }}
                        className="px-3 py-2 text-sm font-mono font-bold text-slate-300 hover:text-white hover:bg-slate-800 transition"
                      >
                        {sym}
                      </button>
                      <button
                        onClick={() => toggleWatchlist(sym)}
                        className="px-2 py-2 text-slate-600 hover:text-rose-400 hover:bg-rose-500/10 transition border-l border-slate-800"
                        title="Remove"
                      >
                  ...
                  <div className="mt-8 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="flex justify-start items-center gap-3 mb-4">
                      <button
                        ...
                        className="bg-slate-800/80 hover:bg-slate-700 text-slate-300 border border-slate-700 px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 shadow-lg backdrop-blur-sm"
                      ...
                      <button
                        ...
                        className="bg-slate-800/80 hover:bg-slate-700 text-slate-300 border border-slate-700 px-4 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg backdrop-blur-sm"
                        title="Refresh Table"
                      >
                              ...
                              <tr key={i} className="hover:bg-slate-800/30 transition group">
                                ...
                                <td className="p-4">
                                  <div className="flex flex-col">
                                    <div className="flex items-center gap-2">
                                      <a href={`https://www.tradingview.com/chart/S83uhZmn/?symbol=${res.ticker}`} target="_blank" rel="noopener noreferrer" className="font-bold text-slate-200 hover:text-amber-400 hover:underline transition cursor-pointer">{res.ticker}</a>
                                    </div>
                                  </div>
                                </td>
                                <td className="p-4">
                                ...
                                <td className="p-4 font-mono text-sm font-medium">
                                  {res.levels?.take_profit_1 ? (
                                    <span className={`${(res.technical_indicators?.current_price || 0) >= ((showDynamicLevels || !res.levels.static_tp1) ? res.levels.take_profit_1 : res.levels.static_tp1) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded' : 'text-emerald-400'}`}>
                                ...
                                <td className="p-4 font-mono text-sm font-medium">
                                  {res.levels?.take_profit_2 ? (
                                    <span className={`${(res.technical_indicators?.current_price || 0) >= ((showDynamicLevels || !res.levels.static_tp2) ? res.levels.take_profit_2 : res.levels.static_tp2) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded' : 'text-emerald-400'}`}>
                                ...
                                <td className="p-4 pr-6">
                                  <div className="flex items-center justify-end gap-3">
                                    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${isBuy ? 'bg-emerald-500 text-slate-950' : res.trading_decision?.action === 'HOLD' ? 'bg-amber-500 text-slate-950' : res.trading_decision?.action === 'WATCH' ? 'bg-indigo-500 text-white' : 'bg-rose-500 text-white'}`}>
                                    ...
                                    <button
                                      ...
                                      className="p-1.5 rounded-lg text-slate-600 hover:text-rose-400 hover:bg-rose-500/20 transition"
              ...
              <>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                  <div>
                    ...
                    <p className="text-slate-500 text-xs mt-1">Auto-scan Yahoo Finance trending stocks</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    <select
                      ...
                      className="bg-slate-950 border border-slate-800 text-slate-300 text-sm rounded-xl px-3 py-2 outline-none focus:border-blue-500 flex-1 sm:flex-none"
                    >
                    ...
                    {screenerResults.length > 0 && (
                      <button
                        onClick={() => setShowDynamicLevels(!showDynamicLevels)}
                        className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 px-3 py-2 rounded-xl text-sm font-bold transition flex items-center gap-2 shrink-0"
                        title={showDynamicLevels ? "Show Static Gann" : "Show Dynamic TP/SL"}
                      >
                        {showDynamicLevels ? 'Show Static Gann' : 'Show Dynamic TP/SL'}
                      </button>
                ...
                {isScreening ? (
                  ...
                  <div className="py-12 text-center border border-dashed border-slate-800 rounded-2xl bg-slate-950/20 text-slate-500 text-sm">
                              ...
                              <tr key={i} className="hover:bg-slate-800/30 transition group">
                                ...
                                <td className="p-4">
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] text-slate-500 line-clamp-1">{res.company_name || res.name}</span>
                                  </div>
                                </td>
                                <td className="p-4">
                                  <span className={`font-bold ${
                                ...
                                <td className="p-4 font-mono text-sm font-medium">
                                  {res.levels?.take_profit_1 ? (
                                    ...
                                    parseFloat(res.tp1) > 0 ? (
                                      <span className={`${parseFloat(res.price) >= parseFloat(res.tp1) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded' : 'text-emerald-400'}`}>
                                        {getCurrencySymbol(res.ticker)} {parseFloat(res.tp1).toFixed(2)}
                                      </span>
                                    ) : '-'
                                  ) : (
                                    parseFloat(res.staticTP1) > 0 ? (
                                      <span className={`${parseFloat(res.price) >= parseFloat(res.staticTP1) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded' : 'text-emerald-400'}`}>
                                ...
                                <td className="p-4 font-mono text-sm font-medium">
                                  {res.levels?.take_profit_2 ? (
                                    ...
                                    parseFloat(res.tp2) > 0 ? (
                                      <span className={`${parseFloat(res.price) >= parseFloat(res.tp2) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded' : 'text-emerald-400'}`}>
                                        {getCurrencySymbol(res.ticker)} {parseFloat(res.tp2).toFixed(2)}
                                      </span>
                                    ) : '-'
                                  ) : (
                                    parseFloat(res.staticTP2) > 0 ? (
                                      <span className={`${parseFloat(res.price) >= parseFloat(res.staticTP2) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded' : 'text-emerald-400'}`}>
                                ...
                                <td className="p-4 pr-6">
                                  <div className="flex items-center justify-end gap-3">
                                    <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${isBuy ? 'bg-emerald-500 text-slate-950' : actionText === 'HOLD' ? 'bg-amber-500 text-slate-950' : actionText === 'WATCH' ? 'bg-indigo-500 text-white' : 'bg-rose-500 text-white'}`}>
                                      {actionText}
                                    </span>
                                    <button
                                      onClick={() => {
            ...
            <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6 backdrop-blur-md relative overflow-hidden">
              ...
              <div className="relative z-10 flex flex-col md:flex-row gap-4 items-end">
                <div className="flex-1">
                  <h3 className="text-2xl font-bold text-slate-200 mb-2">🇺🇸 US Market Sniper</h3>
        ...
        {/* Results Screen */}
        {loading && !result && (
          <div className="flex flex-col items-center justify-center py-20 bg-slate-900/20 border border-slate-800/30 rounded-3xl backdrop-blur-sm animate-pulse">
            <span className="w-10 h-10 border-3 border-slate-700 border-t-blue-500 rounded-full animate-spin mb-4" />
            <p className="text-slate-500 text-sm font-medium font-mono">Running Adaptive Sniper simulation...</p>
          </div>
        )}
        ...
        {result && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-[#0a0a0a] border border-slate-800 rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl relative custom-scrollbar">
              {/* Close Button */}
              <div className="sticky top-0 right-0 w-full flex justify-end p-4 z-10 pointer-events-none">
                <button
                  onClick={() => setResult(null)}
                  className="p-2 bg-slate-900/90 hover:bg-slate-800 text-slate-400 hover:text-slate-200 rounded-full backdrop-blur-md transition-all border border-slate-700 pointer-events-auto shadow-lg"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              return (
                <div className={`border bg-gradient-to-br ${styles.bg} p-6 md:p-8 rounded-3xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6`}>
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="bg-slate-900/60 border border-slate-800 text-slate-300 px-3 py-1 rounded-xl text-xs font-medium flex items-center gap-1.5">
                    ...
                    {result.company_name && (
                      <div className="text-sm text-slate-400 font-sans tracking-wide">
                        {result.company_name}
                      </div>
                    )}
                  ...
                  {result.technical_indicators && (
                    <div className="bg-slate-950/60 border border-slate-800 p-4 rounded-2xl min-w-[150px] text-right flex flex-col justify-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {/* Target Levels Card */}
              <div className="md:col-span-2 bg-slate-900/30 border border-slate-800/80 p-6 rounded-3xl backdrop-blur-md flex flex-col justify-between space-y-6">
                <div>
                  ...
                  {result.trading_decision.action === 'AVOID' ? (
                    <div className="flex flex-col items-center justify-center py-10 text-center border border-dashed border-slate-805 rounded-2xl bg-slate-950/40">
                      <ShieldAlert className="w-10 h-10 text-rose-500/80 mb-3" />
                      <p className="text-slate-400 text-sm font-semibold mb-1">Status: AVOID</p>
                    ...
                    <>
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                        ...
                        <div className="bg-slate-950/50 border border-slate-850 p-3 rounded-2xl">
                          ...
                          <span className="text-[9px] text-slate-500 block mt-1">Sniper Trigger</span>
                        </div>
                        ...
                        <div className="bg-slate-950/50 border border-slate-850 p-3 rounded-2xl">
                          ...
                          <span className={`text-sm sm:text-base font-bold font-mono block w-fit ${(result.technical_indicators?.current_price || 0) >= ((showDynamicLevels || !result.levels.static_tp1) ? result.levels.take_profit_1 : result.levels.static_tp1) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded -ml-1' : 'text-emerald-400'}`}>
                        ...
                        <div className="bg-slate-950/50 border border-slate-850 p-3 rounded-2xl">
                          ...
                          <span className={`text-sm sm:text-base font-bold font-mono block w-fit ${(result.technical_indicators?.current_price || 0) >= ((showDynamicLevels || !result.levels.static_tp2) ? result.levels.take_profit_2 : result.levels.static_tp2) ? 'bg-slate-700/30 text-yellow-400 px-1 rounded -ml-1' : 'text-purple-400'}`}>
                        ...
                        <div className="bg-slate-950/50 border border-slate-850 p-3 rounded-2xl">
                          ...
                          <span className="text-[9px] text-slate-500 block mt-1">3.0x R:R Target</span>
                        </div>
                      </div>

                      {/* Support & Resistance info badge */}
                      {result.levels.support && result.levels.resistance && (
                        <div className="flex flex-wrap gap-3 pt-4 border-t border-slate-850/40 mt-4 text-[11px]">
                          <div className="flex items-center gap-1.5 bg-slate-950/40 border border-slate-850/80 px-3 py-1.5 rounded-xl">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                            <span className="text-slate-500 font-medium">Nearest Support (10D Swing Low):</span>
                            <span className="text-slate-200 font-mono font-semibold">{cSym} {result.levels.support.toFixed(2)}</span>
                          </div>
                          <div className="flex items-center gap-1.5 bg-slate-950/40 border border-slate-850/80 px-3 py-1.5 rounded-xl">
                            <span className="w-1.5 h-1.5 rounded-full bg-rose-400" />
                            <span className="text-slate-500 font-medium">Nearest Resistance (10D Swing High):</span>
                            <span className="text-slate-200 font-mono font-semibold">{cSym} {result.levels.resistance.toFixed(2)}</span>
                          </div>
                        </div>
                      )}
                    </>
                ...
                {result.trading_decision.action !== 'AVOID' && (
                  <div className="bg-slate-950/40 border border-slate-850 p-5 rounded-2xl space-y-3">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-400 font-medium">Risk : Reward Ratio Visualization</span>
                      <span className="text-slate-300 font-semibold font-mono bg-slate-900 px-2 py-0.5 rounded-lg border border-slate-800">
                    ...
                    <div className="relative h-6 w-full mb-1">
                      ...
                      >
                        <span className="bg-slate-800 border border-slate-700 text-[9px] font-bold text-slate-200 px-1.5 py-0.5 rounded shadow-md whitespace-nowrap mb-0.5">
                    ...
                    <div className="relative h-2 bg-slate-850 rounded-full overflow-hidden flex">
                    ...
                    <div className="relative h-4 text-[10px] text-slate-500 font-mono mt-1">
                ...
                {result.technical_indicators?.current_price && (
                  <div className="bg-slate-950/40 border border-slate-850 p-5 rounded-2xl mt-4">
                    <div className="flex justify-between items-center mb-4">
                      <h4 className="text-slate-200 font-bold text-sm flex items-center gap-2">
                        <Target className="w-4 h-4 text-indigo-400" />
                        Gann Square of 9 (Static Crosses)
                      </h4>
                      <div className="flex items-center gap-2 bg-slate-900/50 p-1 rounded-xl border border-slate-800/80">
                        <button 
                          onClick={() => setIsGannIntraday(false)}
                          className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-all ${!isGannIntraday ? 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-500 hover:text-slate-300 border border-transparent'}`}
                        >
                          Swing (1-5 Days)
                        </button>
                        <button 
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="flex flex-wrap items-center gap-3">
                          <div className="flex flex-col gap-1.5 bg-slate-900/50 border border-slate-800/80 px-4 py-2.5 rounded-xl min-w-[90px]">
                            <span className="text-[10px] text-slate-500 font-bold tracking-wider">STOP LOSS</span>
                            <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1.5 bg-slate-900/50 border border-slate-800/80 px-4 py-2.5 rounded-xl min-w-[90px]">
                            <span className="text-[10px] text-slate-500 font-bold tracking-wider">TARGET 1</span>
                            <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1.5 bg-slate-900/50 border border-slate-800/80 px-4 py-2.5 rounded-xl min-w-[90px]">
                            <span className="text-[10px] text-slate-500 font-bold tracking-wider">TARGET 2</span>
                            <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1.5 bg-slate-900/50 border border-slate-800/80 px-4 py-2.5 rounded-xl min-w-[90px]">
                            <span className="text-[10px] text-slate-500 font-bold tracking-wider">TARGET 3</span>
                            <div className="flex items-center gap-2">
                          ...
                          <div className="flex flex-col gap-1.5 bg-slate-900/50 border border-slate-800/80 px-4 py-2.5 rounded-xl min-w-[90px]">
                            <span className="text-[10px] text-slate-500 font-bold tracking-wider">TARGET 4</span>
                            <div className="flex items-center gap-2">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="bg-slate-900/30 border border-slate-800/80 p-6 rounded-3xl backdrop-blur-md flex flex-col justify-between">
                  <div>
                    <h3 className="text-slate-200 font-bold text-lg flex items-center gap-2 mb-4">
                    ...
                    {result.adaptive_sniper ? (
                      <div className="divide-y divide-slate-850">
                        ...
                        <div className="flex justify-between items-center py-2">
                          <span className="text-slate-400 text-xs font-semibold">Trend</span>
                        ...
                        {/* Score */}
                        <div className="flex justify-between items-center py-2">
                          <span className="text-slate-400 text-xs font-semibold">Score</span>
                        ...
                        {/* HTF Bias */}
                        <div className="flex justify-between items-center py-2">
                          <span className="text-slate-400 text-xs font-semibold">HTF Bias</span>
                          <span className={`text-xs font-bold ${
                            result.adaptive_sniper.htf_bias === 'Bullish' ? 'text-emerald-400' :
                            result.adaptive_sniper.htf_bias === 'Bearish' ? 'text-rose-400' : 'text-slate-400'
                          }`}>
                        ...
                        {/* Volatility */}
                        <div className="flex justify-between items-center py-2">
                          <span className="text-slate-400 text-xs font-semibold">Volatility</span>
                          <span className={`text-xs font-bold ${
                            result.adaptive_sniper.volatility === 'High' ? 'text-rose-400' :
                            result.adaptive_sniper.volatility === 'Low' ? 'text-slate-500' : 'text-emerald-400'
                          }`}>
                        ...
                        {/* RSI & ADX */}
                        <div className="flex justify-between items-center py-2">
                          <span className="text-slate-400 text-xs font-semibold">RSI (21) / ADX (14)</span>
                        ...
                        {/* Market / Preset */}
                        <div className="flex justify-between items-center py-2">
                          <span className="text-slate-400 text-xs font-semibold">Exchange / Preset</span>
                      ...
                      <div className="text-slate-500 text-xs py-4 text-center">Dashboard not loaded.</div>
                    )}
                  </div>

                  <div className="pt-4 border-t border-slate-850 mt-4 text-[10px] text-slate-500 flex items-center gap-1">
                    ...
                    <div>
                      ...
                      <div className="divide-y divide-slate-850">
                        ...
                        <div className="flex justify-between items-center py-3">
                          <span className="text-slate-400 text-xs font-semibold">Win Rate (Hit TP1)</span>
                        ...
                        {/* Total Signals */}
                        <div className="flex justify-between items-center py-3">
                          <span className="text-slate-400 text-xs font-semibold">Total Signals</span>
                        ...
                        {/* Wins vs Losses */}
                        <div className="flex justify-between items-center py-3">
                          <span className="text-slate-400 text-xs font-semibold">Wins / Losses</span>
                          <span className="text-xs font-mono font-bold text-slate-200">
            ...
            <div className="bg-slate-900/30 border border-slate-800/80 p-6 rounded-3xl backdrop-blur-md space-y-4">
              <h3 className="text-slate-200 font-bold text-lg flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                <div className="bg-slate-950/40 border border-slate-850 p-5 rounded-2xl space-y-2">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Confluence Analysis</h4>
                ...
                <div className="bg-slate-950/40 border border-slate-850 p-5 rounded-2xl space-y-3">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Trend Structure</h4>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-300 font-medium">Current Structure:</span>
                ...
                {loadingNews ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className={`p-5 rounded-2xl border flex flex-col justify-center items-center text-center gap-2 ${
                      ...
                      <span className="text-xs font-bold uppercase tracking-widest text-slate-400">AI Sentiment</span>
                    ...
                    <div className="md:col-span-2 bg-slate-950/40 border border-slate-850 p-5 rounded-2xl">
                      ...
                      {newsResult.articles && newsResult.articles.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-slate-800/50">
                          <h4 className="text-xs font-bold text-slate-500 mb-2">Recent Headlines</h4>
        ...
        {activeTab === 'calculator' && (
          <div className="bg-slate-900/40 border border-slate-800/80 rounded-3xl p-6 md:p-8 backdrop-blur-xl shadow-2xl mb-8">
            <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50 transition font-mono uppercase"
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex gap-3">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        placeholder="0.00"
                        className="w-full bg-slate-950/50 border border-slate-800 rounded-xl pl-8 pr-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-blue-500/50 transition font-mono"
                      />
                    </div>
                    <button
                  ...
                  {calcCompany && (
                    <div className="text-slate-100/90 text-[11px] sm:text-xs flex items-center gap-1.5 mt-1 font-medium">
                ...
                <div className="flex gap-2">
                  {calcRecent.map(t => (
                    <button
                      ...
                      className="bg-slate-800/50 hover:bg-slate-700/50 border border-slate-700/50 rounded-full px-3 py-1 text-xs font-mono text-slate-300 transition"
                    >
                      {t}
                    </button>
                  ))}
        ...
        {showImportModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl shadow-2xl w-full max-w-lg relative animate-in fade-in zoom-in-95 duration-200">
              <button
              ...
              <textarea
                ...
                className="w-full h-40 bg-slate-950 border border-slate-800 rounded-xl p-4 text-slate-300 text-sm font-mono focus:outline-none focus:border-indigo-500 resize-none mb-6"
              />
              
              <div className="flex justify-end gap-3">
                <button
                  onClick={() => setShowImportModal(false)}
                  className="px-4 py-2 rounded-xl text-sm font-bold text-slate-400 hover:text-slate-200 hover:bg-slate-800 transition"
                >
                  Cancel
                </button>
                <button
